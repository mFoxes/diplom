...
export default class CurrentEmployeeService extends AxiosApi {
	public async getCurrentEmployee(): Promise<Either<AxiosError<IErrorResponse>, ICurrentEmployee>> {
		const req = this._get<ICurrentEmployee>({ url: URL_FACTORY.usersCurrent });
		return this._doApiRequest(req);
	}

	public async getCurrentEmployeeOverdueCount(): Promise<Either<AxiosError<IErrorResponse>, IOverdueCountResponse>> {
		const req = this._get<IOverdueCountResponse>({ url: URL_FACTORY.bookingsOverdue });

		return this._doApiRequest(req);
	}

	public async getCurrentEmployeeOverdue(): Promise<Either<AxiosError<IErrorResponse>, IOverdueListResponse>> {
